...
auto main(int argc, char* argv[]) -> int try {
  cxxopts::Options options("pmemobjbench",
                           "Benchmarking pmem using libpmemobj++");
  // clang-format off
  options.add_options()
    ("h,help", "Print usage")
  ...
  auto stripe_size =
      pretty_bytes::prettyTo<uint64_t>(parsed["stripe"].as<std::string>());
  auto nthreads =
      pretty_bytes::prettyTo<uint64_t>(parsed["nthreads"].as<std::string>());
  auto total_size =
      pretty_bytes::prettyTo<uint64_t>(parsed["total"].as<std::string>());
